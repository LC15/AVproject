...
original_image = cv2.imread('rec3.png')
...
area_bounds = sort_contours(contours)
epsilon = 0.05 * cv2.arcLength(area_bounds[-1], True)
approx_contours = cv2.approxPolyDP(area_bounds[-1], epsilon, True)
...
final_image = cv2.warpPerspective(original_image, transformation, (max_width, max_height))
